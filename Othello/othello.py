...
def value(board, player, depth):
    ...
    if depth == 0 or not legal_moves(board, player):
        return score(board)
    best = max if player == 'X' else min
    return best(
        value(successor(board, player, move), opposite(player), depth - 1) for move in legal_moves(board, player))
...
def best_move(board, player, depth):
    ...
    result = None
    for m in moves:
        new_board = successor(board, player, m)
        v = value(new_board, opposite(player), depth - 1)
